...
```{r read table }
facenet <- read.table("data/facebook_sample_anon.txt")
tail(facenet)
```
*Load data*

```{r generate a graph}
facenet <- graph_from_data_frame(facenet, directed = F)
...
components.page_rank.no <- components.degrees$no
components.page_rank.max <- max(components.degrees$csize / nVertex)
```
...
graph_directed <- graph_from_data_frame(facenet, directed = T)
```
