...
Build adjacency matrix to calculate the connectedness score
```{r adjacency matrix}
adj <- as_adjacency_matrix(facenet, type = "lower",
                           attr = NULL, edges = FALSE, names = TRUE,
                           sparse = F)
connectedness(adj)
```
As we can see the grade of connectivity is very high (=1). Then, we can say this graph has perfect connectivity.
...
Build a function that given a graph removes random N vertex and return the resultant graph of the operation.
...
Build a function that given two lists returns TRUE if equal of FALSE otherwise.
```{r CompareList}
...
```
We can see that by removing the 0.1% of nodes with the highest degrees, "107", "1684", "1912" and "3437", we can split the network in 41 components. The fraction represented by the largest component is 0.9878563.
...
```
CompareList return FALSE, meaning the 0.1% with high degrees are different from the 0.1% with high closeness. Then we proceed to calculate its components number and the fraction.
```{r closeness3}
...
```
This time by removing vertex "107", "58", "428" and "563" with higher closeness have result in fewer components, 12. The fraction represented by the largest component is 0.9972739. This method produced a bigger cluster.
...
```
Now compare the 0.1% high betweenness with the list in degrees.
```{r betweenness2}
...
```
Booth lists are equal, then betweenness result is expected to be equal to degrees, 41 components and equal fraction represented by the largest component is 0.9878563.
...
```
There is no diference between using diferent algorithms of page_rank. All of them produced same vertex list.

Now compare the 0.1% high page_rank scores with the different algorithms, and then with degrees/betweenness and closeness.
```{r page_rank2}
...
```
As we can see, page_rank 0.1% high score vertex list is different from degrees/betweenness and closeness. Then we need to calculate components for the new generated list.

Since there is no difference between algorithms within page_rank, we can use any of the vertex list to produce same components after vertex deletion.
```{r page_rank3}
...
```
Page_rank scored the maximum components, 52. This means that page_rank is able to select the most valuable nodes in the newtwork. The fraction represented by the largest component is 0.983891.
...
Load the facenet graph as a directed graph
...
Calculate Hub and Authority scores for the directed graph
...
plot(facenet_directed,
     ...
     vertex.label = NA)
```
There is too many nodes to see a clear view of the network. But, we can observe that Hubs and Authorities are placed at same side of the net.


Calculate the list of 0.1% high hub_score.
```{r HUBS 2}
vertex.hub <- names(sort(hub_score, decreasing = TRUE)[1:Op1PercentVertex])
vertex.hub
```
Calculate the components and the fraction.
```{r HUBS 3}
...
```
Here we can see that by removing the hubs the network loses its connectivity and got split into 8 different clusters.


Calculate the list of 0.1% high authority_score.
```{r AUTHORITIES 2}
...
```
The 0.1% high score Authorities does not effect the network enough to fragment the network.


#### Page_rank directed score

Calculate the page_rank score but using a directed graph. This time we used arpack algorithm, but behind the scenes we checked that again algorithm does not change the output in a meaningful way to change the 0.1% with high score.
```{r Pagerank score}
...
```
Colors did not work with this one. It's impossible to see where high score nodes are.

Build a list of the 0.1% page_rank high score
```{r page_rank_directed1}
vertex.page_rank <- names(sort(page_rank, decreasing = TRUE)[1:Op1PercentVertex])
vertex.page_rank
```
Calculate the components after removing 0.1% high page_rank score vertex.
...
We check if vertex list is equal across the different methods.
```{r compare lists}
...
components.page_rank.max
```
Like on number of clusters compilation suggest, we can't tell which are the differences apart from hubs that its capable of segment the network into different clusters.
